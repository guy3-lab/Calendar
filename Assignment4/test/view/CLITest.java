...
public class CLITest {
  ...
  public void testInteractiveViewErrorDisplay() {
    ...
    assertTrue("Error output should contain error message", errorOutput.contains(errorMessage));
    assertTrue("Error output should contain 'Error:' prefix", errorOutput.contains("Error:"));

    String standardOutput = outputStream.toString();
    assertEquals("Standard output should be empty for error messages", "", standardOutput);
  }
  ...
  public void testHeadlessExecutionSuccess() {
    ...
    String errorOutput = errorStream.toString();
    assertEquals("No errors should be produced for valid execution", "", errorOutput);
  }
  ...
  public void testHeadlessModeFileNotFound() {
    ...
    String output = outputStream.toString();
    assertFalse("Should not contain success messages", output.contains("Exit command found"));
  }
  ...
  public void testHeadlessErrorHandlingAndRecovery() {
    ...
    String output = outputStream.toString();
    assertNotNull("Output should not be null", output);
    assertTrue("Should contain successful event creation", output.contains("Created event"));
    assertTrue("Should complete with exit", output.contains("Exit command found"));

    String errorOutput = errorStream.toString();
    assertNotNull("Error output should not be null", errorOutput);
    assertTrue("Should contain line number references",
            errorOutput.contains("Line 2:") || errorOutput.contains("Line 3:") ||
                    errorOutput.contains("Line 5:"));
    ...
    assertTrue("Should attempt to execute multiple commands despite errors", executionLines >= 5);
  }
